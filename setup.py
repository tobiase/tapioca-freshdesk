...
def get_author(package):
    ...
    """
    init_py = open(os.path.join(package, "__init__.py")).read()
    return re.search("^__author__ = ['\"]([^'\"]+)['\"]", init_py, re.MULTILINE).group(
        1
    )
...
setup(
    name="tapioca-freshdesk",
    version=get_version(package),
    description="Freshdesk API wrapper using tapioca",
    long_description=readme,
    author=get_author(package),
    author_email=get_email(package),
    url="https://github.com/tobiase/tapioca-freshdesk",
    packages=["tapioca_freshdesk"],
    package_dir={"tapioca_freshdesk": "tapioca_freshdesk"},
